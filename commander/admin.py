...
class CommanderAdmin(admin.ModelAdmin):
	list_display = ('title','slug','command','created','updated','order')
		...
		for c in qs:
			if c.has_filled_command():
				try:
					...
					self.message_user(request, _("Comand %(title)s faild, %(error)s") % {"title": c.title,"error": e})
					count_executed -= 1
			else:
				count_executed -= 1
				logger.warning("Command %s is empty", c.slug)
		self.message_user(request, _("%(ce)s commands from %(cw)s was successfully executed") % {"ce": count_executed,"cw": count_wanted})
	run_commands.short_description = _("Run commands")


admin.site.register(Command, CommanderAdmin)